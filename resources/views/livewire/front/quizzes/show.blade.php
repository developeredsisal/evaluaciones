<div class="mt-4"
     ...
     Tiempo restante para esta pregunta: <span x-text="secondsLeft" class="font-bold"></span> sec.
    <br/><br/>
    <b>Pregunta {{ $currentQuestionIndex+1 }} de {{ $this->questions->count() }}:</b>
    ...
    @if ($currentQuestionIndex < $this->questions->count() - 1)
        <div class="mt-4">
            <button class="btn btn-secondary" x-on:click="{ secondsLeft = {{ config('quiz.secondsPerQuestion') }}; $wire.changeQuestion(); }">siguiente
            </button>
        </div>
    @else
        <div class="mt-4">
            <button class="btn btn-indigo" wire:click="submit">Enviar</button>
        </div>
    @endif
</div>
